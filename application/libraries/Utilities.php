...
class Utilities
{
    ...
    function set_value_session_filter($prefix, $field, $default_value = NULL, $button = 'submit')
    {
        ...
        {
            $conciertos_buscador_fecha_hasta = $this->CI->session->userdata($prefix . '_' . $field);
            //if (empty($conciertos_buscador_fecha_hasta))
            // Es mejor ponerlo así para poder asignar valores iguales a 0 los campos (como para los agentes sin asignar por ejemplo)
            if ($conciertos_buscador_fecha_hasta=='')
            {
